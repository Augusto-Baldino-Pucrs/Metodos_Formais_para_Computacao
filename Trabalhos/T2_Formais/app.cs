// Dafny program app.dfy compiled into C#
...
// Command-line arguments: run app.dfy
// app.dfy
...
class CircularDeque {
  ...
  method Contains(value: int) returns (found: bool)
    ...
    requires size > 0
    ensures Valid()
    ensures found ==> exists k: int {:trigger elements[Index(k)]} :: 0 <= k < size && elements[Index(k)] == value
    ensures !found ==> forall k: int {:trigger elements[Index(k)]} :: 0 <= k < size ==> elements[Index(k)] != value
    decreases value
  {
    var i := 0;
    while i < size
      invariant Valid()
      invariant 0 <= i <= size
      invariant forall k: int {:trigger elements[Index(k)]} :: 0 <= k < i ==> elements[Index(k)] != value
      decreases size - i
    {
  ...
  method PushHead(value: int)
    ...
    ensures elements[head] == value
    ensures head == (old(head) - 1 + capacity) % capacity
    ensures forall i: int {:trigger elements[i]} :: 0 <= i < capacity && i != head ==> elements[i] == old(elements[i])
    ensures old(size) == 0 ==> tail == head
    decreases value
  {
    if size == capacity {
      return;
  ...
  method PushTail(value: int)
    ...
    ensures capacity == old(capacity)
    ensures elements == old(elements)
    ensures elements[tail] == value
    ensures tail == (old(tail) + 1) % capacity
    ensures forall i: int {:trigger elements[i]} :: 0 <= i < capacity && i != tail ==> elements[i] == old(elements[i])
    ensures old(size) == 0 ==> tail == head
    decreases value
  {
    if size == capacity {
  ...
  method PopHead() returns (value: int)
    ...
    ensures capacity == old(capacity)
    ensures elements == old(elements)
    ensures value == old(elements[head])
    ensures head == (old(head) + 1) % capacity
    ensures forall i: int {:trigger elements[i]} :: 0 <= i < capacity && i != head ==> elements[i] == old(elements[i])
    ensures old(size) == 1 ==> tail == -1
  {
    value := elements[head];
  ...
  method PopTail() returns (value: int)
    ...
    ensures capacity == old(capacity)
    ensures elements == old(elements)
    ensures value == old(elements[tail])
    ensures tail == (old(tail) - 1 + capacity) % capacity
    ensures forall i: int {:trigger elements[i]} :: 0 <= i < capacity && i != tail ==> elements[i] == old(elements[i])
    ensures old(size) == 1 ==> head == 0
  {
    value := elements[tail];
    var oldTail := tail;
    ...
    decreases newCapacity
  {
    ...
    for i: int := 0 to size
      ...
      invariant 0 <= i <= size
      invariant forall j: int {:trigger newElements[j]} :: 0 <= j < i ==> newElements[j] == elements[(head + j) % capacity]
    {
...
namespace _module {
  ...
  public partial class __default {
    ...
    {
      CircularDeque _0_deque;
      CircularDeque _nw0 = new CircularDeque();
      _nw0.__ctor(new BigInteger(5));
      _0_deque = _nw0;
      bool _1_empty;
      bool _out0;
      _out0 = (_0_deque).IsEmpty();
      _1_empty = _out0;
      (_0_deque).PushTail(BigInteger.One);
      (_0_deque).PushTail(new BigInteger(2));
      (_0_deque).PushHead(BigInteger.Zero);
      BigInteger _2_size;
      BigInteger _out1;
      _out1 = (_0_deque).Size();
      _2_size = _out1;
      bool _3_contains1;
      bool _out2;
      _out2 = (_0_deque).Contains(BigInteger.One);
      _3_contains1 = _out2;
      bool _4_contains3;
      bool _out3;
      _out3 = (_0_deque).Contains(new BigInteger(3));
      _4_contains3 = _out3;
      BigInteger _5_head;
      BigInteger _out4;
      _out4 = (_0_deque).PopHead();
      _5_head = _out4;
      BigInteger _6_tail;
      BigInteger _out5;
      _out5 = (_0_deque).PopTail();
      _6_tail = _out5;
      (_0_deque).SetCapacity(new BigInteger(10));
      BigInteger _7_cap;
      BigInteger _out6;
      _out6 = (_0_deque).Capacity();
      _7_cap = _out6;
    }
  }

  public partial class CircularDeque {
    public CircularDeque() {
      this.capacity = BigInteger.Zero;
      this.size = BigInteger.Zero;
      this.head = BigInteger.Zero;
      this.tail = BigInteger.Zero;
      this.elements = new BigInteger[0];
    }
    public BigInteger capacity {get; set;}
    public BigInteger size {get; set;}
    public BigInteger head {get; set;}
    public BigInteger tail {get; set;}
    public BigInteger[] elements {get; set;}
    public BigInteger Index(BigInteger k) {
      return Dafny.Helpers.EuclideanModulus((this.head) + (k), this.capacity);
    }
    public void __ctor(BigInteger capacity)
    {
      (this).capacity = capacity;
      (this).size = BigInteger.Zero;
      (this).head = BigInteger.Zero;
      (this).tail = new BigInteger(-1);
      BigInteger[] _nw0 = new BigInteger[Dafny.Helpers.ToIntChecked(capacity, "array size exceeds memory limit")];
      (this).elements = _nw0;
    }
    public bool IsEmpty()
    {
      bool empty = false;
      empty = (this.size).Sign == 0;
      return empty;
    }
    public bool IsFull()
    {
    ...
    {
      BigInteger maxCapacity = BigInteger.Zero;
    ...
    public bool Contains(BigInteger @value)
    {
      ...
      while ((_0_i) < (this.size)) {
        if (((this.elements)[(int)((this).Index(_0_i))]) == (@value)) {
          found = true;
          return found;
        }
    ...
    public void PushHead(BigInteger @value)
    {
      if ((this.size) == (this.capacity)) {
        return ;
      }
      BigInteger _0_oldHead;
      _0_oldHead = this.head;
      (this).head = Dafny.Helpers.EuclideanModulus(((this.head) - (BigInteger.One)) + (this.capacity), this.capacity);
      BigInteger[] _arr0 = this.elements;
      BigInteger _index0 = this.head;
      _arr0[(int)(_index0)] = @value;
      if ((this.size).Sign == 0) {
        (this).tail = this.head;
      } else {
      }
    ...
    public void PushTail(BigInteger @value)
    {
      if ((this.size) == (this.capacity)) {
        return ;
      }
      BigInteger _0_oldTail;
      _0_oldTail = this.tail;
      (this).tail = Dafny.Helpers.EuclideanModulus((this.tail) + (BigInteger.One), this.capacity);
      BigInteger[] _arr0 = this.elements;
      BigInteger _index0 = this.tail;
      _arr0[(int)(_index0)] = @value;
      if ((this.size).Sign == 0) {
        (this).head = this.tail;
      } else {
    ...
    public BigInteger PopHead()
    {
      BigInteger @value = BigInteger.Zero;
      @value = (this.elements)[(int)(this.head)];
      (this).head = Dafny.Helpers.EuclideanModulus((this.head) + (BigInteger.One), this.capacity);
      if ((this.size) == (BigInteger.One)) {
        (this).tail = new BigInteger(-1);
      }
      (this).size = (this.size) - (BigInteger.One);
      return @value;
    }
    public BigInteger PopTail()
    {
      BigInteger @value = BigInteger.Zero;
      @value = (this.elements)[(int)(this.tail)];
      BigInteger _0_oldTail;
      _0_oldTail = this.tail;
      (this).tail = Dafny.Helpers.EuclideanModulus(((this.tail) - (BigInteger.One)) + (this.capacity), this.capacity);
      if ((this.size) == (BigInteger.One)) {
        (this).head = BigInteger.Zero;
      } else {
      }
    ...
    {
      BigInteger[] _0_newElements;
      BigInteger[] _nw0 = new BigInteger[Dafny.Helpers.ToIntChecked(newCapacity, "array size exceeds memory limit")];
      _0_newElements = _nw0;
      BigInteger _hi0 = this.size;
      for (BigInteger _1_i = BigInteger.Zero; _1_i < _hi0; _1_i++) {
        (_0_newElements)[(int)((_1_i))] = (this.elements)[(int)(Dafny.Helpers.EuclideanModulus((this.head) + (_1_i), this.capacity))];
      }
      (this).elements = _0_newElements;
      (this).capacity = newCapacity;
      (this).head = BigInteger.Zero;
      (this).tail = (this.size) - (BigInteger.One);
    }
  }
